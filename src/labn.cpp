...
labN::labN(int numInput, int numHidden, int numOutput) {
	_numInput = numInput;
...
std::vector <double> labN::ComputeHiddenOutputs(std::vector <double> xValues) {
...
std::vector <double> labN::ComputeOutputs(std::vector <double> xValues) {
...
double labN::HyperTanFunction(double x) {
	if (x < -20.0) return -1.0;
	else if (x > 20.0) return 1.0;
	else return tanh(x);
}

std::vector <double> labN::Softmax(std::vector <double> oSums)  {
	double max = oSums[0];
...
void labN::ComputeGradient(std::vector <double> tValues) {
...
void labN::UpdateWeights(std::vector <double> tValues, double learnRate) {
	for (int i = 0; i < _hiddenWeights.size(); ++i) {
...
void labN::UpdateBias(std::vector <double> tValues, double learnRate) {
...
void labN::BackWard(std::vector <double> tValues, double learnRate) {
...
double labN::MeanCrossEntropyError(std::vector <std::vector <double>> trainData)
{
	double sumError = 0.0;
...
void labN::TrainNetwork(std::vector<std::vector<double>> trainData, int maxEprochs, double learnRate, double crossError) {
	int epoch = 0;
	std::vector<double> xValues(_numInput, 0); 
	std::vector<double> tValues(_numOutput, 0); 
...
double labN::Accuracy(std::vector<std::vector<double>> testData) {
	int numCorrect = 0;
	int numWrong = 0;
	std::vector<double> xValues(_numInput, 0);
	std::vector<double> tValues(_numOutput, 0);
...
int labN::MaxIndex(std::vector<double> vec) {
	int bigIndex = 0;
...
void labN::Shuffle(std::vector<int> sequence) {
	for (int i = 0; i < sequence.size(); i ++) {
